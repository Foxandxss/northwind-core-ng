...
[IdeaBlade](https://www.ideablade.com/) has found works well for many projects.
...
Our sample is going to have client and server projects, and database scripts.  
So create the following directory structure:

    NorthwindCore/  # or your project name
      client/
      dbscripts/
      server/
...
1. Copy contents of the dbscripts folder from the repo into your own dbscripts folder.
...
1. Create the server project.  
    ...
    - Set the Location to the "server" directory that you created above
...
```
In the `Configure` method, configure CORS and turn on MVC:
```
...
Compile the solution, then open a command prompt in the `server` directory.  Then run:

`dotnet NorthwindServer\bin\Debug\netcoreapp2.2\NorthwindServer.dll metadata`

This should produce a big blob of JSON:
```
...
Compile and run the solution.  It should open a browser and attempt to open a page on the default port, e.g https://localhost:44345 ,
...
Next, open a command prompt in the `NorthwindCore/client` directory, and follow the steps below:
...
Now you should have a `NorthwindCore/client/northwind` directory containing the Angular app.  Try it out:
...
Start by adding the npm packages.  In the `client/northwind` directory, run:
...
When developing our app, it's helpful to have TypeScript classes to represent the entity data
that comes from the server.  We'll use the metadata from our NorthwindServer project to 
generate the TypeScript classes.

Start by opening a command prompt in the `server` directory.  Then run:

`dotnet NorthwindServer\bin\Debug\netcoreapp2.2\NorthwindServer.dll metadata > metadata.json`

That will create a `metadata.json` file in the `server` directory.

To turn the metadata into entities, we need to write a script.  In the `client/northwind` directory,
...
This should create files in the `northwind/src/app/model` directory:
```
customer.ts
entity-model.ts
metadata.ts
...
Edit `\client\northwind\src\app\app.module.ts`.  At the top of the file, add the following imports:
```
...
export const environment = {
  production: false,
  breezeApiRoot: 'https://localhost:44345/api/breeze',
};
```
...
> In a real project, now would be a good time to start creating submodules.  For this demo, we will keep everything in one
module for simplicity.

Create the file `northwind/src/app/entity-manager-provider.ts`.  In the file, put:
```
...
Now create a component to display some customer data.  Open a command prompt in the `client/northwind` directory, and execute the command:

`northwind>`ng generate component customer`
...
Try it now: if the app is not already running, open a command prompt in the `client/northwind` directory and run:
